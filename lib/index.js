...
export function getResourcesHandler (i18next, options = {}) {
  ...
  return function (req, res) {
    ...
    options.contentType(res, 'json')
    if (options.cache !== undefined ? options.cache : process.env.NODE_ENV === 'production') {
      options.setHeader('Cache-Control', 'public, max-age=' + maxAge)
      options.setHeader('Expires', (new Date(new Date().getTime() + maxAge * 1000)).toUTCString())
    } else {
      options.setHeader('Pragma', 'no-cache')
      options.setHeader('Cache-Control', 'no-cache')
    }
