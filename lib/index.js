...
export function freshPlugin (i18next, options) {
  return async (ctx) => {
    handle(i18next, { ...options, attachLocals: true })(ctx)
...
export function getResourcesHandler (i18next, options = {}) {
  ...
  return function (rq, rs) {
    ...
    // lng goes through the strict check (rejects `/`); ns goes through
    // the loose check (allows `/` so nested-namespace layouts like `a/b`
    // that map to `public/locales/en/a/b.json` keep working). Directory
    // escape via `..` / `\` / control chars is still blocked on both.
...
export function missingKeyHandler (i18next, options = {}) {
  ...
  return function (rq, rs) {
    ...
    // Same check getResourcesHandler applies, but this is the write path:
    // these params are forwarded to backendConnector.saveMissing() and end
    // up as the `{{lng}}`/`{{ns}}` segments of the backend's addPath, so an
    // unvalidated value is an arbitrary file write on i18next-fs-backend
    // (route params arrive percent-decoded, e.g. `..%2f..%2ftmp`).
    if (!utils.isSafeLangIdentifier(lng) || !utils.isSafeNsIdentifier(ns)) {
      options.setStatus(res, 400)
      return options.send(
        res,
        'i18next-http-middleware:: unsafe lng/ns value'
      )
    }
