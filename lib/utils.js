...
// Defense-in-depth check for `missingKeyHandler`. The literal-key denylist
// catches `{"__proto__": ...}` but misses dotted variants like
// `{"__proto__.polluted": "x"}` — backends that split the missing-key string
// on a configured keySeparator (e.g. i18next-fs-backend's writeFile()) would
// hand that key to an unguarded setPath and walk into Object.prototype.
// Reject any key whose segments — under the configured keySeparator (default
// `.`) — include an unsafe key. keySeparator === false means "no splitting",
// in which case only the literal-key check applies.
...
export function setPath (object, path, newValue) {
  ...
  while (stack.length > 1) {
    ...
    // walk landed on Object.prototype. (GHSA-r4j4-5cw9-pwgj)
