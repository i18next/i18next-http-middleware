...
class LanguageDetector {
  ...
  detect (req, res, detectionOrder) {
    ...
    detectionOrder.forEach(detectorName => {
      ...
      // Detected languages come from untrusted request input (querystring,
      // path, cookie, session, header) and are forwarded to
      // changeLanguage()/loadLanguages(), which hand them to the backend as
      // a `{{lng}}` path/URL segment. hasXSS only covers markup payloads, so
      // the identifier check is what blocks directory escape (`..`, `/`,
      // `\`), control characters and prototype keys. Applied *after*
      // convertDetectedLanguage so a custom converter cannot reintroduce an
      // unsafe value; non-string converter output is dropped (fail-closed).
